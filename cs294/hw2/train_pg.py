...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        for layer_id in range(1, n_layers + 1):
            cur_output_size = output_size if layer_id == n_layers else size
            cur_activation = output_activation if layer_id == n_layers else activation
            prev_layer = tf.layers.dense(
                prev_layer, cur_output_size, activation=cur_activation, name='dense_layer_%d' % layer_id)
        return prev_layer
    ...
    for itr in range(n_iter):
        ...
        if nn_baseline:
            ...
            sess.run(baseline_update_op, feed_dict={
                     sy_ob_no: ob_no, baseline_targets: q_n})
        ...
        loss_v, _ = sess.run([loss, update_op], feed_dict)
