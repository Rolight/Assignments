...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for idx in range(len(labels)):
            windowed_data.append((reduce(lambda x, y: x + y,
                sentence[idx: idx + 2 * window_size + 1]),
                labels[idx]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            shape=(None, self.config.n_window_features), dtype=tf.int32)
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedings = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.reshape(
            tf.nn.embedding_lookup(embedings,
                                   self.input_placeholder),
            [-1, self.config.embed_size * self.config.n_window_features])
        ### END YOUR CODE
        return embeddings
