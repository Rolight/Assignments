...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.Variable(np.zeros(b1_shape), dtype=tf.float32)
        U = tf.Variable(initializer(U_shape))
        b2 = tf.Variable(np.zeros(b2_shape), dtype=tf.float32)

        h = tf.nn.relu(tf.matmul(x, W) + b1)
        h_drop = tf.nn.dropout(h, self.dropout_placeholder)
        pred = tf.matmul(h_drop, U) + b2
        # l2 regularization
        alpha = self.config.regular_param / 2
        l2reg = tf.reduce_sum(W ** 2) * alpha
        ### END YOUR CODE
        return pred, l2reg
