...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable(
                name='W_x', dtype=tf.float32,
                shape=(self.input_size, self.state_size),
                initializer=initializer)
            W_h = tf.get_variable(
                name='W_h', dtype=tf.float32,
                shape=(self.state_size, self.output_size),
                initializer=initializer)
            b = tf.get_variable('b', dtype=tf.float32,
                                shape=(1, self.output_size),
                                initializer=initializer)
            input_signal = tf.matmul(inputs, W_x)
            historical_signal = tf.matmul(state, W_h)
            new_state = tf.nn.sigmoid(input_signal + historical_signal + b)
            ### END YOUR CODE ###
