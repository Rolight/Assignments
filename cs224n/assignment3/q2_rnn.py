...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        masking = [True] * len(sentence) + [False] * pad_length
        sentence += [zero_vector] * pad_length
        labels += [zero_label] * pad_length
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            shape=(None, self.config.max_length, self.config.n_features),
            dtype=tf.int32)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                x_t = tf.transpose(x, [1, 0, 2])[time_step]
                output_t, hidden_t = cell(x_t, h)
...
def do_test2(args):
    ...
    with tf.Graph().as_default():
        ...
        logger.info('Model fit finished in %.2f seconds', time.time() - start)
