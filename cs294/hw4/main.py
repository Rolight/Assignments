...
def sample(env,
    ...
    for _ in range(num_paths):
        ...
        while len(path['observations']) < horizon:
            ...
            if render:
                env.render()
            path['observations'].append(last_obs)
...
def compute_normalization(data):
    ...
    """ YOUR CODE HERE """
    all_obs = np.concatenate([p['observations'] for p in data])
    all_next_obs = np.concatenate([p['next_observations'] for p in data])
    all_action = np.concatenate([p['actions'] for p in data])

    mean_obs = np.mean(all_obs)
    std_obs = np.std(all_obs)

    mean_deltas = np.mean(all_next_obs - all_obs)
    std_deltas = np.mean(all_next_obs - all_obs)

    mean_action = np.mean(all_action)
    std_action = np.std(all_action)
